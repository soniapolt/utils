function faceLassoTrim_sherlock(expt,subj,stimNum,minR2,ppd,ROI,weight)
...
reg.ppd = ppd;
reg.faceRad = ceil(3.2*reg.ppd/2);
...
center = round(im.centers(t,:)./(im.ppd)*reg.ppd);
ind = [center(1)-reg.faceRad:center(1)+reg.faceRad; center(2)-reg.faceRad:center(2)+reg.faceRad];
    

%%% resize our stimulus image
invIm = imresize(condAvg(:,:,cStim(t)),[reg.imSize,reg.imSize]);
invIm=invIm-min(invIm(:)); %normalize to range from 0 to 1
invIm=invIm./max(invIm(:));
reg.invIm{t} = invIm(ind(1,:),ind(2,:));
...
for v = 1:numVox
   fullCov = flipud(PRF(X,Y,fits(bCond).vox(v).XYdeg(1)*reg.ppd+mid,fits(bCond).vox(v).XYdeg(2)*reg.ppd+mid,fits(bCond).vox(v).size*reg.ppd));
   reg.coverage(v,:,:) = fullCov(ind(1,:),ind(2,:));
...
fprintf('Starting trimLasso %s %s stim #%d...will save to %s\n',ROI,reg.subjs{1},stimNum,...
...
checkDir(regDir);
save([regDir '/' regFile 'Trim_' num2str(stimNum) '.mat'],'reg');
fprintf('lassoTrim %s %s stim #%d finished at %s, took %s mins\n\n',reg.subjs{1},ROI,stimNum,datestr(now),num2str(reg.timeFit/60));
end
