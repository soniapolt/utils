function h = plotCoverage(vox,color,leg,ppd,res,fontSize)
% vox is a structure of v voxels with  a params field

set(gca,'YDir','reverse');
    for v = 1:length(vox)
        h = plotCircle(vox(v).params(2),-vox(v).params(1),2*vox(v).params(3)/sqrt(vox(v).params(5))/2,color,.25,'edge'); % center
        hold on;
